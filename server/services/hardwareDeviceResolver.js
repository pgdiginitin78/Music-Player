...
export function getPhysicalMacAddress() {
  ...
  try {
      ...
      if (Array.isArray(netInterface)) {
        for (const item of netInterface) {
          if (
            ...
            console.log(
              `[HARDWARE RESOLVER] Resolved Physical Hardware MAC Address: ${cachedHardwareMac} (via OS Interface: ${name})`,
            );
            return cachedHardwareMac;
          }
        }
    ...
    console.warn(
      "[HARDWARE RESOLVER WARN] OS network interface error:",
      err.message,
    );
  }
...
export function getHardwareIdentityInfo() {
  ...
  const maskedMac =
    mac.length > 8 ? `${mac.slice(0, 7)}****${mac.slice(-4)}` : mac;
