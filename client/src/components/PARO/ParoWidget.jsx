...
import {
  ...
  getParoDiagnostics,
} from "../../services/deviceCapabilities.js";
...
export default function ParoWidget() {
  ...
  const [debugInfo, setDebugInfo] = useState(getParoDiagnostics());
  ...
  useEffect(() => {
    const unsubscribe = paroVoiceEngine.subscribe((event) => {
      ...
      if (event.debug) {
        setDebugInfo({ ...getParoDiagnostics(), ...event.debug });
      }
    });

    return () => {
      paroVoiceEngine.stopParoSession();
      unsubscribe();
    };
  }, []);
  ...
  const handleMicTap = async () => {
    if (isOpen) {
      ...
      paroVoiceEngine.startParoSession(getPlayerState(), playerControlsRef.current);
      setIsOpen(true);
      setErrorMessage("");
      setTranscript("");
    }
  ...
  const handleSendText = (textOverride) => {
    ...
    paroVoiceEngine.handleIncomingTranscript(
      text,
      getPlayerState(),
      playerControlsRef.current,
    );
  ...
  const getStateText = () => {
    switch (state) {
      case "idle":
        return supportsContinuousWakeWord
          ? 'Say "Hey PARO" or tap mic'
          : "Tap mic to talk to PARO";
      case "wake_detected":
        return "Wake word detected!";
      case "listening_for_command":
        return "Listening... Speak now";
      case "requesting_permission":
        return "Allowing microphone...";
      case "processing":
        return "Processing speech...";
      case "understanding":
        return "Understanding intent...";
      case "executing":
        return "Executing music action...";
      case "speaking":
        return "PARO is speaking...";
      case "error":
        return errorMessage || "Microphone unavailable";
      default:
        return supportsContinuousWakeWord
          ? 'Say "Hey PARO" or tap mic'
          : "Tap mic to talk to PARO";
        ...
        <div className="pointer-events-auto w-80 sm:w-96 p-4 rounded-3xl bg-slate-950/95 backdrop-blur-2xl border border-violet-500/30 shadow-2xl flex flex-col gap-3.5 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="px-3.5 py-2.5 rounded-2xl bg-slate-900/80 border border-white/10 flex flex-col gap-1 text-xs">
              ...
              {lastReply && (
                <div className="text-white">
                  <span className="text-violet-400 font-semibold mr-1">
                    PARO:
                  </span>
            ...
            <div className="relative flex items-center justify-center">
              {(state === "listening_for_command" ||
                ...
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                  <div
                    className="absolute w-14 h-14 rounded-full border border-violet-400/60 animate-paro-ring-fast-1"
                    style={{ boxShadow: `0 0 20px ${glowColor}` }}
                  />
                  <div
                    className="absolute w-14 h-14 rounded-full border border-violet-400/40 animate-paro-ring-fast-2"
                    style={{ boxShadow: `0 0 16px ${glowColor}` }}
                  />
                  <div
                    className="absolute w-14 h-14 rounded-full border border-violet-400/25 animate-paro-ring-fast-3"
                  />
                </div>
              )}
              ...
              >
                ...
                state === "understanding" ? (
                  <img
                    src={loadingIcon}
                    alt=""
                    className="w-6 h-6 invert animate-spin"
                  />
                ) : (
          ...
          <div className="pt-2 border-t border-white/10 flex flex-col gap-1 text-[10px] font-mono text-gray-400">
            ...
            >
              <span>
                {showDebug
                  ? "▼ HIDE PARO DIAGNOSTIC"
                  : "▶ SHOW PARO DIAGNOSTIC"}
              </span>
              ...
              <div className="p-2.5 rounded-2xl bg-slate-900/90 border border-white/10 space-y-1 text-[10px] font-mono">
                <div>
                  ...
                  <strong className="text-white">
                    {debugInfo.platform || "Desktop / Android"}
                  </strong>
                </div>
                <div>
                  iOS / iPadOS:{" "}
                  <strong className={debugInfo.isIOS ? "text-amber-400" : "text-emerald-400"}>
                    {String(debugInfo.isIOS)}
                  </strong>
                </div>
                <div>
                  PWA Standalone:{" "}
                  <strong className={debugInfo.isPWA ? "text-emerald-400" : "text-gray-400"}>
                    {String(debugInfo.isPWA)}
                  </strong>
                </div>
                <div>
                  HTTPS Context:{" "}
                  <strong className={debugInfo.isSecureContext ? "text-emerald-400" : "text-red-400"}>
                    {String(debugInfo.isSecureContext)}
                  </strong>
                </div>
                <div>
                  Microphone:{" "}
                  <strong className={debugInfo.microphone === "AVAILABLE" || debugInfo.microphoneAvailable === "Available" ? "text-emerald-400" : "text-red-400"}>
                    {debugInfo.microphone || debugInfo.microphoneAvailable || "UNKNOWN"}
                  </strong>
                </div>
                <div>
                  Speech Recognition:{" "}
                  <strong className="text-violet-300">
                    {debugInfo.speechRecognitionSupported || debugInfo.speechRecognition || "NOT_INITIALIZED"}
                  </strong>
                </div>
                <div>
                  Media Session API:{" "}
                  <strong className="text-emerald-300">
                    {debugInfo.mediaSessionSupported || "Supported"}
                  </strong>
                </div>
                <div>
                  Wake Word Mode:{" "}
                  <strong className="text-amber-300">
                    {debugInfo.wakeWordMode || "Standard"}
                  </strong>
                </div>
                ...
                <div className="pt-1 border-t border-white/10 space-y-0.5">
                  <div>
                    ...
                    <span className="text-gray-200">
                      "{debugInfo.lastTranscript || transcript || "None"}"
                    </span>
                  </div>
                  <div>
                    Matched song:{" "}
                    <span className="text-emerald-300">
                      {debugInfo.matchedSong || "None"}
                    </span>
                  </div>
                  <div>
                    Player:{" "}
                    <span className="text-cyan-300">
                      {debugInfo.player || "IDLE"}
                    </span>
                  </div>
                </div>

                {/* Natural Young Adult Hindi Female Voice Sample Buttons */}
                <div className="pt-1.5 border-t border-white/10 flex flex-col gap-1">
                  <span className="text-violet-400 font-semibold">
                    Test Natural Hindi Female Voice Preview:
                  </span>
                  <div className="flex flex-wrap gap-1">
                    <button
                      type="button"
                      onClick={() =>
                        voiceService.previewParoVoice(
                          "हाँ नितिन, बताओ... कौन सा गाना चलाऊँ?",
                        )
                      }
                    ...
                    <button
                      ...
                      onClick={() =>
                        voiceService.previewParoVoice(
                          "Okay नितिन, Kesariya चला रही हूँ.",
                        )
                      }
                      className="px-2 py-0.5 rounded bg-violet-900/60 hover:bg-violet-800 text-white text-[9px]"
                    >
